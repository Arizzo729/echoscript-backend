...
# System deps: lxml + Postgres headers + curl for healthcheck
RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential gcc libpq-dev libxml2-dev libxslt1-dev git curl \
...
# Simple healthcheck (must be single line)
