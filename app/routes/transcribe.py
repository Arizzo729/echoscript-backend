...
async def transcribe(
    ...
    try:
        ...
        # Calculate duration from file metadata or mock
        try:
            ...
            # Fallback if moviepy fails or not installed
            duration_seconds = 60 
        
        # Check usage limit
        transcripts = db.query(Transcript).filter(Transcript.user_id == current_user.id).all()
        used_minutes = sum(((t.duration or 0) / 60) for t in transcripts)
        if used_minutes + (duration_seconds / 60) > 120:
