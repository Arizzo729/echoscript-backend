...
def _safe_user_from_token(data: Dict[str, Any], db: Session) -> User:
    sub = data.get("sub")
    if not sub:
        raise HTTPException(status_code=401, detail="Invalid token")
...
def me(request: Request, db: Session = Depends(get_db)) -> MeOut:
    ...
    user = _safe_user_from_token(data, db)

    email = data.get("email") or user.email
    return MeOut(id=user.id, email=email, mode="jwt")
...
def refresh(request: Request, response: Response, db: Session = Depends(get_db)) -> LoginOut:
    ...
    new_token = _create_jwt({"sub": str(user.id), "email": user.email})
