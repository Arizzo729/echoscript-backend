...
def on_startup():
    log.info("=== Backend Startup Started ===")
    
    STORAGE_DIR.mkdir(parents=True, exist_ok=True)
    log.info(f"Storage directory ready: {STORAGE_DIR}")
    ...
    # Load heavy ASR model if requested — don't fail startup if model import fails
    if ASGI_LOAD_MODEL and ASGI_ENABLE_TRANSCRIBE:
        ...
        try:
            ...
            log.exception("Whisper model loading failed during startup; continuing without model: %s", e)
    else:
        log.info(f"Skipping Whisper load (ASGI_LOAD_MODEL={ASGI_LOAD_MODEL}, ASGI_ENABLE_TRANSCRIBE={ASGI_ENABLE_TRANSCRIBE})")

    # Mount application routes (router imports already guarded per-module)
    log.info("Mounting application routes...")
    try:
        _mount_all_routes()
        log.info("All routes mounted successfully")
    except Exception as e:
        log.exception("Mounting routes failed during startup: %s", e)

    # Log which email providers appear to be configured (don't print secrets)
    try:
        have_resend = bool(os.getenv("RESEND_API_KEY"))
        have_sendgrid = bool(os.getenv("SENDGRID_API_KEY"))
        have_smtp = all(bool(os.getenv(k)) for k in ["SMTP_HOST", "SMTP_PORT", "SMTP_USER", "SMTP_PASS", "SMTP_FROM"])
        log.info("Email config: Resend=%s SendGrid=%s SMTP=%s", have_resend, have_sendgrid, have_smtp)
    except Exception:
        pass
...
def _add_diagnostics():
    ...
    def diag_email():
        ...
        have_sendgrid = bool(os.getenv("SENDGRID_API_KEY"))
        have_smtp = all(bool(os.getenv(k)) for k in ["SMTP_HOST", "SMTP_PORT", "SMTP_USER", "SMTP_PASS", "SMTP_FROM"])
        # Determine best available mode: Resend -> SendGrid -> SMTP -> NONE
        if have_resend:
            mode = "Resend"
        elif have_sendgrid:
            mode = "SendGrid"
        ...
        return {
            ...
            "sendgrid_api_key_present": have_sendgrid,
